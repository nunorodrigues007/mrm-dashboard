    ...
    for offset in [0, 1]:
        for month in [1, 6]:
            for d in range(31, 24, -1):
                try:
                    dt = date(yr + offset, month, d)
                    if dt.weekday() == 4 and dt >= date.today():
...
_today_d = date.today()
_friday  = _today_d - timedelta(days=(_today_d.weekday() - 4) % 7)
_is_sem  = _friday.month in {1, 6} and (_friday + timedelta(days=7)).month != _friday.month
...
if _is_sem:
    ...
    _rb_alert  = "INACTIVE"
    _rb_status = "No structural regime change detected. Holding current positions."
    _rb_color  = "#122008"
    _rb_border = "#34D058"
    _rb_icon   = "●"
...
7. After the CIO Verdict section, include a PORTFOLIO REBALANCE STATUS section using the data above.
   Style it as a distinct box with background {_rb_color}, border-left 4px solid {_rb_border}, icon {_rb_icon}.
...
if not clean_subscribers:
    clean_subscribers = ["usmrm@proton.me"]
